 ...
 */
export const extractMetadataFromTTML = (
  ttmlContent: string,
): { ncmIds: number[]; musicNames: string[]; artists: string[] } => {
        ...
        limit(async () => {
          try {
            ...
            if (ncmIds.length > 0 || musicNames.length > 0) {
              await cache.set(ncmIds, musicNames, artists, filePath, fileStat.mtimeMs, {
                autoSave: false,
              });
              hasChanges = true;
              scannedCount++;
